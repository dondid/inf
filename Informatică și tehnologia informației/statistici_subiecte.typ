...
Pentru a optimiza procesul de învățare, această secțiune identifică sistematic *ce structuri de probleme s-au repetat, de câte ori și în ce sesiuni specifice de examen* (Titularizare sau Definitivat). Analiza acoperă toate cele trei mari componente ale examenului: *Algoritmică și Programare*, *Baze de Date și TIC*, respectiv *Didactică și Evaluare*.

#v(3mm)
= 2. Tabelul Recurențelor în Algoritmică și Programare (Subiectul I & II)

Tabelul de mai jos inventariază tipurile de algoritmi care apar recurent sau folosesc aceeași tehnică de bază:
...
#table(
  ...
  [*Același tipar logic* (sumarea cifrelor repetată până la o singură cifră sau sume de cifre)],
  ...
  [*Același subalgoritm de bază* (calcularea sumei divizorilor proprii în $O(sqrt(N))$)],
  ...
  [*Recurent cu același tipar* (definitare, pași trace în tabel, implementare cod)],
  ...
  [*Aceeași structură teoretică* (definiții, trace-uri și codul clasic)],

  [*Grafuri Hamiltoniene & Backtracking*
   (Cicluri hamiltoniene, generări)],
  [2 ori],
  [- 2014 Definitivat (Sub. I.1)\ - 2026 Model Titularizare (Sub. I.1)],
  [*Recurent cu același tipar* (cerință de backtracking pentru determinarea ciclului hamiltonian)],
  ...
  [*Foarte apropiat* (interclasare cu doi pointeri în timp liniar $O(N + M)$)]
)
...
#table(
  ...
  [*Aceeași abordare teoretică* (explicarea regulilor de dependență funcțională tranzitivă și parțială)],
  ...
  [*Recurent cu același tipar* (definirea componentelor interne și a fazelor ciclului de instrucțiuni)],

  [*Serviciul E-mail (Rețele)*
   (SMTP, POP3, IMAP, structură mesaje)],
  [2 ori],
  [- 2016 Titularizare (Sub. I.2)\ - 2026 Model Titularizare (Sub. I.2)],
  [*Recurent cu același tipar* (comunicare asincronă, rolul protocoalelor, aplicații software specifice)]
)
...
#table(
  ...
  [*Recurent cu același tipar* (cerințe privind obiectivele, criteriile de evaluare a produsului și evaluarea activității echipei)],
  ...
  [*Recurent cu același tipar* (cerințe similare pentru baze de date sau editoare de texte)],

  [*Învățarea prin descoperire*
   (Metodă didactică activă)],
  [3 ori],
  [- 2013 Titularizare (Sub. I.3)\ - 2026 Definitivat (Sub. III.1)\ - 2026 Titularizare (Sub. III.1)],
  [*Recurent cu același tipar* (caracteristici, tipuri și realizarea scenariului didactic detaliat)],

  [*Problematizarea*
   (Crearea de situații-problemă)],
  [2 ori],
  [- 2016 Titularizare (Sub. III.1)\ - 2025 Definitivat (Sub. III.1)],
  [*Recurent cu același tipar* (etapele problematizării, activitatea profesorului și a elevilor pe algoritmi)]
)
